...
public class Partie {
    ...
    public boolean combat(Joueur now) {
        int[] rendu = attackJoueur();
        int idAttaque = rendu[0];
        int iddefense = rendu[1];
        int atq;
        int def = 0;
        atq = now.attaquerTerritoire(idAttaque);
        for (Joueur joueur : Joueurs) {
            for (int j = 0; j < joueur.ListeTerritoire.size(); j++)
                if (iddefense == joueur.ListeTerritoire.get(j).id) {
                    if (now == joueur) {
                        //exceptions
                    } else {
                        def = joueur.attaquerTerritoire(joueur.ListeTerritoire.get(j).id);
                    }
                }
        }
        if (atq - def > 0) {
    ...
    public Joueur reherchejoueur(int id){
        for (Joueur joueur : Joueurs) {
            for (Territoire terri: joueur.ListeTerritoire){
                if(terri.id == id){
                    return joueur;
                }
            }
